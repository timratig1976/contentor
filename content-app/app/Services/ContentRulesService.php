...
class ContentRulesService
{
     ...
     */
    public function pickPainCluster(string $text, Unit $unit): ?array
    {
        ...
        $defaultKey = $unit->default_cluster_key;
        foreach ($clusters as $cluster) {
     ...
     */
    public function guessIcp(string $input, ?string $explicitIcp, Unit $unit): string
    {
        if ($explicitIcp) return $explicitIcp;

        $lower = mb_strtolower($input);
        foreach ($unit->icp_guesser as $entry) {
            if (isset($entry['match']) && preg_match('/' . $entry['match'] . '/i', $lower)) {
                return $entry['icp'];
            }
        }

        return $unit->default_icp;
    }
     ...
     */
    public function enforceTone(string $text, ?string $format, Unit $unit, ?array $strategyCtx = null): string
    {
        $output = $text;
     ...
     */
    public function checkToneViolations(string $text, Unit $unit, ?array $strategyCtx = null): array
    {
        $violations = [];
        $raw = $text;
